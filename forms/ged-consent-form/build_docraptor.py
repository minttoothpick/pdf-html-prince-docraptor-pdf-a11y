...
HTML_FILE = HERE / "ged-consent-form.html"
CSS_FILE = HERE / "ged-consent-form.css"
...
parser = argparse.ArgumentParser(
    description="Build the OTC GED consent form with DocRaptor."
)
...
if css_replacements != 1:
    sys.exit(
        "Could not find the ged-consent-form.css <link> element "
        "in ged-consent-form.html."
    )
...
if test_mode:
    ...
    output_file = output_dir / "ged-consent-form-test.pdf"
    mode_label = "TEST"
else:
    output_dir = PRODUCTION_OUTPUT_DIR
    output_file = output_dir / "ged-consent-form.pdf"
...
payload = {
    "type": "pdf",
    "test": test_mode,
    "name": "Orange Technical College - GED Consent for Release of Confidential Information",
    # Keep using the same pipeline that produced the successful
    # transcript-request PDF/UA output.
    "pipeline": "10.1",
...
print()
print("Form: GED Consent for Release of Confidential Information")
