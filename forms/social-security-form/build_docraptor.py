...
HTML_FILE = HERE / "social-security-form.html"
CSS_FILE = HERE / "social-security-form.css"
...
parser = argparse.ArgumentParser(
    description="Build the OTC Social Security Number Form with DocRaptor."
)
...
stylesheet_pattern = re.compile(
    r'<link\b'
    r'(?=[^>]*\brel=["\']stylesheet["\'])'
    r'(?=[^>]*\bhref=["\']social-security-form\.css["\'])'
    r'[^>]*>',
    flags=re.IGNORECASE,
)
...
if css_replacements != 1:
    sys.exit(
        "Could not find the social-security-form.css <link> element "
        "in social-security-form.html."
    )
...
if test_mode:
    ...
    output_file = output_dir / "social-security-form-test.pdf"
    mode_label = "TEST"
else:
    output_dir = PRODUCTION_OUTPUT_DIR
    output_file = output_dir / "social-security-form.pdf"
...
payload = {
    "type": "pdf",
    "test": test_mode,
    "name": "Orange Technical College - Social Security Number Form",
    "pipeline": "10.1",
...
print()
print("Form: Social Security Number Form")
